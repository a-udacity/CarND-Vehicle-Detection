...
def color_hist(img, num_bins=32, bins_range=(0, 256)):
    channel1_hist = np.histogram(img[:, :, 0], bins=num_bins, range=bins_range)
    channel2_hist = np.histogram(img[:, :, 1], bins=num_bins, range=bins_range)
    channel3_hist = np.histogram(img[:, :, 2], bins=num_bins, range=bins_range)
...
def extract_features(images, color_space='RGB', spatial_size=(32, 32), hist_bins=32, orient=9, pix_per_cell=8,
    ...
    for image in images:
        ...
        # histogram features
        image_features.append(color_hist(feature_image, num_bins=hist_bins))
        # hog features
...
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    if hist_feat:
        # Apply color_hist()
        hist_features = color_hist(feature_image, num_bins=hist_bins,
